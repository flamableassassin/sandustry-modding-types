# Sandkit API quick reference

This is a compact, copy-friendly index of the Sandkit API v1 signatures, split
by main and worker runtime. Use the generated TypeDoc namespace pages when you
need linked parameter and return types. The declaration set is still evolving,
so validate unresolved or `unknown` shapes against the current game.

See [Practical Sandkit API examples](api-examples.md) for complete usage
patterns built from these signatures.
